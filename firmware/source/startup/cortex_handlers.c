...
void HardFault_Handler           ( void ) __attribute__ ((weak, alias("HardFault_DUMMY")));
...
uint32_t getStackSize(void)
{
	return (uint32_t) &__StackTop - (uint32_t) &__StackStart;
}

uint32_t getStackUsed(void)
{
	uint32_t *ptr;
	uint32_t startofStack = ((uint32_t) &__StackStart )+ 8; //assume first 8 bytes are used by heap
	uint32_t count=0;
	ptr = ((uint32_t *) &__StackTop)-8; //do not use the stack top but some 8 bytes below
	ptr = (uint32_t *) ((uint32_t) ptr & 0xFFFFFFF0UL); //make lower 8 bits zero.
	while ((uint32_t) ptr > startofStack)
	{
		if (*ptr == 0xDEADBEEF)
		{
			count++;
			if (count >=10) //wait until we have seen our tag 10 times in a row
			{
				ptr=ptr-count;
				break;
			}
	...
	return (uint32_t) &__StackTop - (uint32_t) ptr;
}

uint32_t getStackStartOff(void)
{
	return (uint32_t) &__StackStart - HSRAM_ADDR;
...
void Reset_Handler(void)
{
	...
	end=((uint32_t) &__StackStart) + 8;
	//fill the stack
	ptr = (uint32_t *) (&ptr - 64); //get address of the stack for this function
	ptr = (uint32_t *) ((uint32_t) ptr & 0xFFFFFFF0UL); //make lower 8 bits of address zero
