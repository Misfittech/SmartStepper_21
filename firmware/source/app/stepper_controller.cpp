...
void StepperCtrl::updateParamsFromNVM(void)
{
	...
	}else
	{
		...
		nvmWriteSystemParms((SystemParams_t &)systemParams);
	}
	...
	} else
	{
		...
		nvmWriteMotorParms((MotorParams_t &)motorParams);
	}
...
float StepperCtrl::measureStepSize(void)
{
	...
	WARNING("motor wiring %d",motorParams.motorWiring);
	stepperDriver.setRotationDirection(motorParams.motorWiring);
	/////////////////////////////////////////
	...
	deg=ANGLE_T0_DEGREES(angle2-angle1);
	...
	LOG("angle delta %f %d (%d %d)",deg,(angle2-angle1),angle1,angle2 );
...
bool StepperCtrl::home(int dir)
{
	...
	while (!done)
	{
		...
		//delay_ms(100);
		mean=sampleMeanEncoder(10);

		float deg;
		deg=ANGLE_T0_DEGREES((mean-lastMean));
		if (deg <-180)
		{
			deg=deg+360.0;
		}
...
bool StepperCtrl::calibrateEncoder(void)
{
	...
	while(!done)
	{
		...
		if (j>1)
		{
			float deg;
			x=(mean-lastMean);
			deg=ANGLE_T0_DEGREES(x);
			if (ABS(deg)>300)
			{
				deg=deg+360;
			}
			LOG("step size deg %f",deg);
			if (abs(deg)<0.9)
			{
...
bool StepperCtrl::vpidFeedback(int64_t desiredLoc, int64_t currentLoc, Control_t *ptrCtrl)
{
	...
	if (enableFeedback) //if ((micros()-lastCall)>(updateRate/10))
	{
		...
		Iterm += (vPID.Ki * error);
		if (Iterm>(16UL*4096UL*CTRL_PID_SCALING *(int64_t)motorParams.currentMa))
		{
			Iterm=(16UL*4096UL*CTRL_PID_SCALING *(int64_t)motorParams.currentMa);
		}
		if (Iterm<-(16UL*4096UL*CTRL_PID_SCALING *(int64_t)motorParams.currentMa))
		{
			Iterm=-(16UL*4096UL*CTRL_PID_SCALING*(int64_t)motorParams.currentMa);
...
bool StepperCtrl::simpleFeedback(int64_t desiredLoc, int64_t currentLoc, Control_t *ptrCtrl)
{
	...
	if (enableFeedback)
	{
		...
		kp_error=(kp * error)/CTRL_PID_SCALING;
		if (1)//((kp * abs(error))/CTRL_PID_SCALING)<(fullStep))
		{
			iTerm+=(sPID.Ki*error);
...
//		}
//		
		u=kp_error+x+(sPID.Kd *(error-lastError))/CTRL_PID_SCALING;
		...
		if (u>(fullStep))
		{
			...
			if (kp_error <u)
			{
				iTerm=(u*CTRL_PID_SCALING)-kp_error;
			}else
			{
				iTerm=0;
			}
