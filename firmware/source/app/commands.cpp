...
static int geterror_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	...
	_snprintf(str,sizeof(str),"%0.2f",f);
	//ftoa(f,str,2,'f');
...
static int errorpin_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	if (argc==1)
	{
		...
		memcpy(&params,(const void *)&NVM->SystemParams, sizeof(SystemParams_t) );
...
static int eepromerror_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	...
	_snprintf(str,sizeof(str),"%0.2f",deg);
	//ftoa(deg,str,2,'f');
...
static int stepsperrotation_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	...
	if (argc == 1)
	{
		...
		if (x==200 || x==400)
		{
			...
			memcpy(&motorParams,(const void *)&NVM->motorParams, sizeof(motorParams) );
...
static int ctrlmode_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	...
	if (argc == 1)
	{
		...
		if (x<=4)
		{
			...
			memcpy(&systemParams,(const void *)&NVM->SystemParams, sizeof(systemParams) );
...
static int errorlimit_cmd(sCmdUart *ptrUart,int argc, char * argv[])
{
	...
	if (argc == 0)
	{
		...
		_snprintf(str,sizeof(str),"%0.2f",x);
		//ftoa(x,str,2,'f');
...
void commandsInit(CharDevice *ptr_uart, CharDevice *ptrRs485)
{
	//Ptr_uart=ptr_uart;
	CommandInit(&CommandsUart, ptr_uart); //set up the UART structure
	//CommandInit(&RS485Uart, ptrRs485); //set up the UART structure
}
