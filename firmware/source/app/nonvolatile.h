...
typedef struct {
	...
	uint32_t rs485_addr; //address of the RS485 port
	uint32_t crc;
...
static uint32_t ParametersCrc(void *ptrParam, size_t param_size)
{
	uint32_t crc=0;
	crc32(ptrParam, param_size-4, &crc);
	return crc;
}

static bool ParametersValid(void *ptrParam, size_t param_size)
{
	uint32_t crc=0;
	uint32_t *ptrCrc=(uint32_t *)(((uint8_t *)ptrParam) + param_size-4);

	if (((uint32_t)ptrCrc)%4)
	{
		ERROR("Pointer to CRC not on word boundry");
		return false;
	}
	crc=ParametersCrc(ptrParam,param_size);
	if (crc == *ptrCrc)
	{
		return true;
	}
...
#ifdef NZS_FAST_CAL
extern  uint16_t  NVM_flash[16767];
#else
extern  const uint16_t  NVM_flash[256];
#endif
#define NVM ((volatile nvm_t *)NVM_flash)
