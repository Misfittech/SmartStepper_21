...
void CalibrationTable::printCalTable(void)
{
	...
	{
		SysLogPrintf("%d",(uint16_t)table[i].value);
		SysLogPrintf(",");
	}
	SysLogPrintf("\n\r");
}
...
void CalibrationTable::saveToFlash(void)
{
	...
	memcpy(&data, (const void *) &NVM->CalibrationTable,sizeof(data));
	createFastCal();
...
bool CalibrationTable::flashGood(void)
{
	...
	return NVM->CalibrationTable.status==1;
}
...
void CalibrationTable::createFastCal(void)
{
	...
	//update the checksum
	flashWrite((void *)&NVM->FastCal.checkSum,&cs,sizeof(uint16_t));
	fastCalVaild=true;
