...
#ifdef NZS_FAST_CAL
__attribute__((__aligned__(FLASH_BLOCK_SIZE)))  __attribute__ ((section (".nvm" ))) uint16_t NVM_flash[16767];  //allocates 33280 bytes
#else
__attribute__((__aligned__(FLASH_BLOCK_SIZE))) __attribute__ ((section (".nvm" ))) const uint16_t NVM_flash[256];  //allocates 512 bytes
#endif
...
{
	bool x=true;
	flashWrite((void *)&NVM->CalibrationTable,ptrData,size);
	return true;
}
...
bool nvmWrite_sPID(float Kp, float Ki, float Kd)
{
	...
	pid.Ki=Ki;
	pid.Kd=Kd;
	pid.crc=ParametersCrc(&pid,sizeof(pid));
...
{
	motorParams.crc=ParametersCrc(&motorParams,sizeof(motorParams));
...
bool nvmErase(void)
{
	...
	flashWrite((void *)&NVM->sPID.crc ,&data,sizeof(data));
	flashWrite((void *)&NVM->vPID.crc ,&data,sizeof(data));
	flashWrite((void *)&NVM->pPID.crc ,&data,sizeof(data));
	flashWrite((void *)&NVM->motorParams.crc ,&data,sizeof(data));
	flashWrite((void *)&NVM->SystemParams.crc ,&data,sizeof(data));
#ifdef NZS_FAST_CAL
