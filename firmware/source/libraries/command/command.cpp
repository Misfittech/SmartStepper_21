...
int CommandInit(sCmdUart *ptrCmdUart, CharDevice *ptrUart)
{
	...
	ptrCmdUart->buffIndex=0;
	ptrCmdUart->lastErrorCode=0;
	return 0;
}
...
int CommandPrintf(sCmdUart *ptrUart, const char *fmt, ...)
{
    ...
    return ptrUart->ptrUart->write((uint8_t *)vastr, (uint8_t)ret);
...
unsigned int CommandParse(sCmdUart *ptrUart,sCommand *ptrCmds, char *str, char delimitor )
{
	...
	//if user types $? or errorlevel return the last error code. 
	if (strcicmp(buff,"$?")==0 || strcicmp(buff,"ERRORLEVEL")==0) 
	{
		CommandPrintf(ptrUart,PSTR("%d\n\r"),ptrUart->lastErrorCode);
		return 0;
	}
...
*/
		...
		if (strlen(buff)==strlen(cmd_list.name))
		{
...
#endif
			{
				...
				//return 1;
				ptrUart->lastErrorCode = (*cmd_list.function)(ptrUart,numArgs,ptrArgv);
				return ptrUart->lastErrorCode;
			}
		}
		i=i+1;
		memcpy(&cmd_list, &ptrCmds[i], sizeof(sCommand));
...
int CommandProcess(sCmdUart *ptrUart,sCommand *ptrCmds, char delimitor, const char *cmdPrompt)
{
	...
	if (strstr(ptrUart->buffer,ANSI_UP)) //up arrow
	{
		...
		CommandPrintf(ptrUart,PSTR("\n\r%s\033[K"),cmdPrompt);
         ...
         i=ptrUart->histUpIndex;
         if (ptrUart->histUpIndex >0)
         {
        	 ptrUart->histUpIndex--;
         }else
         {
        	 ptrUart->histUpIndex=CMD_HISTORY-1;
         }

		 while (strlen(ptrUart->bufferHist[ptrUart->histUpIndex])==0 && ptrUart->histUpIndex>0)
		 {
			 ptrUart->histUpIndex--;
		 }
