...
void UHD_Init(void)
{
	...
	/* Set up the USB DP/DM pins */
 	...
 	PORT->Group[0].PMUX[PIN_PA24H_USB_DM/2].reg |= PIN_PA24H_USB_DM << (4 * (PIN_PA24H_USB_DM & 0x01u));
 	PORT->Group[0].PINCFG[PIN_PA25H_USB_DP].bit.PMUXEN = 1;
 	PORT->Group[0].PMUX[PIN_PA25H_USB_DP/2].reg &= ~(0xF << (4 * (PIN_PA25H_USB_DP & 0x01u)));
 	PORT->Group[0].PMUX[PIN_PA25H_USB_DP/2].reg |= PIN_PA25H_USB_DP << (4 * (PIN_PA25H_USB_DP & 0x01u));
