...
extern "C" void USB_SetHandler(void (*new_usb_isr)(void));
...
const uint8_t STRING_PRODUCT[] = "Nano Zero";
...
const uint8_t STRING_MANUFACTURER[] = "MisfitTech.net";
...
void USBDeviceClass::init()
{
	...
	GCLK->PCHCTRL[USB_GCLK_ID].reg = GENERIC_CLOCK_GENERATOR_48M | (1 << GCLK_PCHCTRL_CHEN_Pos);
#else
...
uint32_t USBDeviceClass::send(uint32_t ep, const void *data, uint32_t len)
{
	...
	while (len != 0)
	{
		if (usbd.epBank1IsReady(ep)) {
			...
			uint32_t timeout = 120ul*(TX_TIMEOUT_MS * 1000) / 23;
